...
pub trait Dependencies {
    ...
    async fn request(scope: &ScopeView) -> anyhow::Result<Self>
    where
    ...
    async fn request_opt(scope: &ScopeView) -> anyhow::Result<Self>
    where
        Self: 'static + Clone + Send + Sync,
    {
        scope
    ...
    async fn link(scope: &ScopeView) -> anyhow::Result<Self>
    where
        Self: 'static + Clone + Send + Sync,
    {
...
impl Dependencies for () {
    async fn request(_scope: &ScopeView) -> anyhow::Result<Self> {
        Ok(())
    }

    async fn request_opt(_scope: &ScopeView) -> anyhow::Result<Self> {
        Ok(())
    }

    async fn link(_scope: &ScopeView) -> anyhow::Result<Self> {
        Ok(())
    }
}
